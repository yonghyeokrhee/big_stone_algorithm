...
def BFS(x, y) -> None:
    ...
    while q:
        ...
        for i in range(4):
            ny = y + dy[i]
            nx = x + dx[i]
            if ny < N and nx < N and nx >= 0 and ny >= 0 and not v[ny][
                nx] and check(ny, nx, y, x):
                v[ny][nx] = 1
                q.append((nx, ny))
                tot += arr[ny][nx]
                cnt += 1
...
while flag:
    ...
    for j in range(N):
        for i in range(N):
            if not v[j][i] and (i, j) not in united_check:
                ret = max(BFS(i, j), ret)

    # 이민 하기
    united_check = united.copy()
    for u, m in list(zip(united,
                         mig)):  # 반드시 연합국이 있는 나라들에 대해서만 연산해야 한다. united와 mig의 길이가 같아야 한다.
        i, j = u
        arr[j][i] = m
